...
function getLevelLabel($level) {
    return match ((int)$level) {
        ...
        3 => '<span class="badge bg-info">Mahasiswa</span>',
        4 => '<span class="badge bg-warning text-dark">Pengurus</span>',
...
// Bangun WHERE clause - Tampilkan level 2, 3, 4 (bukan level 1/SuperAdmin)
$where = "u.level IN (2,3,4)";
...
$sql = "
    SELECT
        ...
        u.id_ormawa,
        u.created_at,
        u.program_studi,  /* <--- DITAMBAHKAN */
        u.angkatan,       /* <--- DITAMBAHKAN */
        o.nama_ormawa
    FROM user u
    ...
    <div class="row mb-3">
        <div class="col-md-3">
            <select class="form-select" id="filterJenis">
                ...
                <option value="2" <?= $filter_level === '2' ? 'selected' : ''; ?>>Admin (Ketua Ormawa)</option>
                <option value="3" <?= $filter_level === '3' ? 'selected' : ''; ?>>Mahasiswa</option>
                <option value="4" <?= $filter_level === '4' ? 'selected' : ''; ?>>Pengurus</option>
            </select>
        </div>
    </div>
                    ...
                    <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>Level</th>
                                <th>Ormawa</th>
                                <th>Prodi</th> <th>Angkatan</th>      <th>Email</th>
                                <th>Terdaftar</th>
                                <th>Aksi</th>
                            </tr>
                        </thead>
                        <tbody id="accountsTableBody">
                            <?php $no = 1; foreach ($accounts as $acc): ?>
                                <tr>
                                    ...
                                    <td class="search-col">
                                        <?php
                                        // Level 3 (Mahasiswa) boleh NULL, tampilkan "-"
                                        if ($acc['level'] == 3 && empty($acc['nama_ormawa'])) {
                                            echo '<span class="text-muted">-</span>';
                                        } else {
                                            echo htmlspecialchars($acc['nama_ormawa'] ?? '–');
                                        }
                                        ?>
                                    </td>
                                    <td class="search-col"><small><?= htmlspecialchars($acc['program_studi'] ?? '-'); ?></small></td> <td class="search-col"><small><?= htmlspecialchars($acc['angkatan'] ?? '-'); ?></small></td>        <td class="search-col"><?= htmlspecialchars($acc['email']); ?></td>
                                    <td>
                                    ...
                                    <td>
                                        <button class="btn btn-warning btn-circle btn-sm edit-btn"
                                            ...
                                            data-ormawa-id="<?= (int)($acc['id_ormawa'] ?? 0); ?>"
                                            title="Edit">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        let count = 0;
        rows.forEach(row => {
            // Mengambil semua teks dari kolom yang memiliki class .search-col
