...
// --- PERUBAHAN: Query difilter untuk jenis_form = 'anggota' dan JOIN ormawa ---
...
$form_fields = [];
if ($active_form_id > 0) {
    // --- PERUBAHAN: Pastikan form yang di-load adalah tipe 'anggota' dan JOIN ormawa ---
    $form_detail_query = "SELECT fi.id, fi.judul, fi.deskripsi, fi.gambar, fi.status, fi.user_id, o.nama_ormawa FROM form_info fi JOIN user u ON fi.user_id = u.id JOIN ormawa o ON u.id_ormawa = o.id WHERE fi.id = ? AND fi.jenis_form = 'anggota'";
